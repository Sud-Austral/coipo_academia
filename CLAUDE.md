...
| Documento | Contenido |
|---|---|
| `docs/TRASLADO.md` | Fase F0: mover el volcado y los 12 GB al servidor |
...
4. **Rotar DOS contraseñas, no una.** La del rol `iam` que estuvo expuesta en el
   `setup_bd.sql` original (hoy el archivo del repositorio, `db/setup_bd.sql`, lleva solo un
   marcador), y **la del rol `academia`**:
   `admin/tool/dbtransfer/cli/migrate.php` la recibe como argumento de línea de comandos, así
   que quedó visible con un `ps` para cualquiera con acceso al contenedor mientras corría la
   migración. Es una limitación de la herramienta de Moodle. Conviene rotarlas juntas, porque
   se parecen entre sí. `ALTER ROLE ... PASSWORD` en 172.31.2.40, actualizar el `.env` con
   `chmod 600` y recrear los contenedores.
